...
function getPlannedDateTime(dateStr) {
  if (!dateStr) return "";
  if (dateStr.includes("T")) { const [dt, t] = dateStr.split("T"); const [y, m, d] = dt.split("-"); return `${d}/${m}/${y} ${t}:00`; }
  const now = new Date(); let fd = dateStr;
...
function parseDate(dateStr) {
  ...
  if (p.length === 3) { if (p[0].length === 4) return new Date(p[0], p[1]-1, p[2]); return new Date(p[2], p[1]-1, p[0]); }
...
async function getFilteredLeads(sheets, user) {
  try {
    ...
    const filteredLeads = []; const doerTag = getDoerTag(user);

    rows.forEach((row, index) => {
      const plannedDate = row[20]?row[20].trim():""; const status = row[22]?row[22].trim():"";
      const oldRemarks = row[11]?row[11].trim():""; const previousRemarksDate = row[13]?row[13].trim():"";
      const previousRemarks = row[19]?row[19].trim():""; const latestRemarks = row[24]?row[24].trim():"";
      const followupCount = row[25]?parseInt(row[25].trim(),10)||0:0;
      const doer = row[38]?row[38].trim():""; const fsrDoer = row[39]?row[39].trim():"";
      const showRow = !status || status.trim().toLowerCase() === "rescheduled";
      if (showRow && plannedDate) {
        if (doerTag && doer !== doerTag) return;
        filteredLeads.push({ rowIndex:index+8, sheetName:NBD_SHEET_NAME, uniqueId:row[1]||"", customerName:row[2]||"", customerContact:row[3]||"", interestedIn:row[4]||"", projectSelection:row[5]||"", leadSource:row[6]||"", leadGenNumber:row[7]||"", leadGenName:row[8]||"", importantNote:row[10]?row[10].trim():"", plannedDate, status:status||"Pending", followupCount, remarks:latestRemarks||previousRemarks||oldRemarks, oldRemarks, previousRemarks, previousRemarksDate, doer, fsrDoer });
      }
    });
...
router.get("/list", async (req, res) => {
  ...
  catch (error) { res.status(500).json({ success:false, error:"Failed", message:error.message }); }
});
...
router.post("/update", async (req, res) => {
  try {
    ...
    try { const cr = await req.sheets.spreadsheets.values.get({ spreadsheetId:SPREADSHEET_ID, range:`'${NBD_SHEET_NAME}'!Z${rowIndex}` }); currentFollowupCount = parseInt(cr.data.values?.[0]?.[0])||0; } catch(e){}
    ...
    if (rescheduleDate && String(rescheduleDate).trim() !== "") {
      updates.push({ range:`'${NBD_SHEET_NAME}'!U${rowIndex}`, values:[[getPlannedDateTime(rescheduleDate)]] });
      ...
      if (leadInfo) await appendToNotInterestedSheet(req.sheets, leadInfo, "Step 2 - Field Visit", notInterestedReason||"", req.user?.email);
    } else {
      updates.push({ range:`'${NBD_SHEET_NAME}'!V${rowIndex}`, values:[[timestamp]] });
      updates.push({ range:`'${NBD_SHEET_NAME}'!W${rowIndex}`, values:[[status||"Done"]] });
      if (req.user && req.user.assignedModule === "fsr") { const fc = getFSRCode(req.user); if (fc) updates.push({ range:`'${NBD_SHEET_NAME}'!AN${rowIndex}`, values:[[fc]] }); }
    }
    ...
    if (leadInfo) await appendToLogger(req.sheets, leadInfo, "Step 2 - Field Visit", status||"Done", remarks||"", req.user?.email);

    res.json({ success:true, message: rescheduleDate ? "Rescheduled" : status==="Not Interested" ? "Marked Not Interested" : "Field Visit Done", newFollowupCount });
