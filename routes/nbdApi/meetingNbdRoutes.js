...
function getPlannedDateTime(dateStr) {
  ...
  if (dateStr.includes("T")) { const [d,t] = dateStr.split("T"); const [y,m,dd] = d.split("-"); const [hh,mm] = t.split(":"); return `${dd}/${m}/${y} ${hh}:${mm}:00`; }
  if (dateStr.includes("-")) { const [y,m,d] = dateStr.split("-"); return `${d}/${m}/${y} 00:00:00`; }
  return dateStr;
}

function parseDate(dateStr) { if (!dateStr) return new Date(0); const p = dateStr.split(/[\/\-]/); if (p.length===3) { if (p[0].length===4) return new Date(p[0],p[1]-1,p[2]); return new Date(p[2],p[1]-1,p[0]); } return new Date(dateStr); }
...
async function getFilteredLeads(sheets, user) {
  const response = await sheets.spreadsheets.values.get({ spreadsheetId: SPREADSHEET_ID, range: `'${SHEET_NAME}'!A8:AN` });
  const rows = response.data.values || [];
  const filtered = []; const doerTag = getDoerTag(user), fsrDoerTag = getFSRDoerTag(user);

  rows.forEach((row, index) => {
    const plannedDate = row[33]?row[33].trim():""; let status = row[35]?row[35].trim():"";
    const doer = row[38]?row[38].trim():"", fsrDoer = row[39]?row[39].trim():"";
    const oldRemarks = row[11]?row[11].trim():"", previousRemarksDate = row[13]?row[13].trim():"";
    const previousRemarks = row[19]?row[19].trim():"", latestOldRemarksDate = row[21]?row[21].trim():"";
    const latestOldRemarks = row[24]?row[24].trim():"", recentRemarksDate = row[27]?row[27].trim():"";
    const recentRemarks = row[32]?row[32].trim():"", currentRemarks = row[37]?row[37].trim():"";
    const displayRemarks = currentRemarks || recentRemarks || latestOldRemarks || previousRemarks || oldRemarks;

    const showRow = plannedDate && (!status || status.toLowerCase()==="rescheduled" || status.toLowerCase()==="next field visit required");
    if (!showRow) return;
    if (doerTag && doer !== doerTag) return;
    if (fsrDoerTag && fsrDoer !== fsrDoerTag) return;
    if (!status.trim()) status = "Pending";

    filtered.push({ rowIndex:index+8, sheetName:SHEET_NAME, uniqueId:row[1]||"", customerName:row[2]||"", customerContact:row[3]||"", interestedIn:row[4]||"", projectSelection:row[5]||"", leadSource:row[6]||"", leadGenNumber:row[7]||"", leadGenName:row[8]||"", plannedDate, status, fsrDoer, remarks:displayRemarks, oldRemarks, previousRemarks, previousRemarksDate, latestOldRemarks, latestOldRemarksDate, recentRemarks, recentRemarksDate, doer });
  });
  return filtered;
}

router.get("/list", async (req, res) => {
  try { const leads = await getFilteredLeads(req.sheets, req.user); leads.sort((a,b) => parseDate(a.plannedDate)-parseDate(b.plannedDate)); res.json({ success:true, data:leads, total:leads.length }); }
...
router.post("/update", async (req, res) => {
  try {
    ...
    const timestamp = getCurrentTimestamp(); const updates = [];
    let currentFollowupCount = 0;
    try { const cr = await req.sheets.spreadsheets.values.get({ spreadsheetId:SPREADSHEET_ID, range:`'${SHEET_NAME}'!Z${rowIndex}` }); currentFollowupCount = parseInt(cr.data.values?.[0]?.[0])||0; } catch(e){}
    ...
    if (rescheduleDate && String(rescheduleDate).trim() !== "") {
      ...
      if (leadInfo) await appendToNotInterestedSheet(req.sheets, leadInfo, "Step 4 - Meeting", notInterestedReason||"", req.user?.email);
    } else {
      updates.push({ range:`'${SHEET_NAME}'!AI${rowIndex}`, values:[[timestamp]] });
    ...
    if (remarks && String(remarks).trim() !== "") {
      const appended = await buildAppendedRemarks(req.sheets, SHEET_NAME, `AL${rowIndex}`, remarks);
      if (appended) updates.push({ range:`'${SHEET_NAME}'!AL${rowIndex}`, values:[[appended]] });
    }
    ...
    if (leadInfo) await appendToLogger(req.sheets, leadInfo, "Step 4 - Meeting", status||"Done", remarks||"", req.user?.email);

    res.json({ success:true, message: rescheduleDate?"Rescheduled" : status==="Next Field Visit Required"?"Next Field Visit Scheduled" : ["Not Interested","Negotiation Failed","Deal Not Done"].includes(status)?"Marked as "+status : "Updated", newFollowupCount });
  } catch (err) { res.status(500).json({ success:false, error:err.message }); }
});
